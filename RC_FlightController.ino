 ...
 * ║  FlySky FS-iA6B iBUS  +  MPU-9250  +  BMP280  +  GPS  +  HUZZAH32 ║
...
#define PIN_GPS_RX    13   // GPS TXD → GPIO13 (UART1 RX)
#define PIN_GPS_TX    17   // GPS RXD ← GPIO17 (UART1 TX, optional)

// ─────────────────────────────────────────────────────────────
//  NOTE: GPIO17 is shared between iBUS TX (unused) and GPS TX (unused).
//  Both are TX-only sides that are never actually driven.
//  No conflict — safe.
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
//  Shared flight state — v2.2 adds GPS
// ─────────────────────────────────────────────────────────────
struct FlightState {
...
static TaskHandle_t hTaskGPS    = nullptr;   // ← NEW
...
// ─────────────────────────────────────────────────────────────
//  Wi-Fi telemetry provider — v2.2 adds GPS fields
// ─────────────────────────────────────────────────────────────
static const char* flightModeToString(FlightMode m) {
    switch(m){ case FlightMode::DISARMED: return "DISARMED"; case FlightMode::ANGLE: return "ANGLE";
               case FlightMode::ACRO: return "ACRO"; case FlightMode::FAILSAFE: return "FAILSAFE"; default: return "UNKNOWN"; }
}
...
static bool provideTelemetry(TelemetryPacket& out) {
    ...
    TuningState t;
    if (xSemaphoreTake(g_tuneMutex, pdMS_TO_TICKS(5)) == pdTRUE) { t = g_tuning; xSemaphoreGive(g_tuneMutex); }
    ...
    out.throttle=s.rc.throttle; out.rc_roll=s.rc.roll; out.rc_pitch=s.rc.pitch; out.rc_yaw=s.rc.yaw;
    out.rc_hz=rcReceiver.getFrameRate();
    out.motor_fl=s.motorFL; out.motor_fr=s.motorFR; out.motor_rl=s.motorRL; out.motor_rr=s.motorRR;
    ...
    out.cpu_core0_pct=s.cpuCore0_pct; out.cpu_core1_pct=s.cpuCore1_pct; out.cpu_valid=s.cpuValid;
...
static void handleTune(const TunePacket& in) {
    bool armed=false;
    if(xSemaphoreTake(g_flightMutex,pdMS_TO_TICKS(5))==pdTRUE){ armed=g_state.armed; xSemaphoreGive(g_flightMutex); }
...
static void runAutonomousCalibration()
{
    ...
    while(millis()<end){
        ...
        if(millis()-lp>=10000){calLogf("[CAL] Mag — %lus remaining",(unsigned long)((end-millis())/1000));lp=millis();}
        delay(10);
    }
...
// ─────────────────────────────────────────────────────────────
//  taskGPS — NEW in v2.2
//  Runs on Core 0 at priority 1. Calls gps.update() as fast as
//  possible (GPS outputs at 1 Hz but NMEA bytes trickle in
//  continuously at 9600 baud — must drain often).
//  Writes into g_state.gps under g_flightMutex.
// ─────────────────────────────────────────────────────────────
static void taskGPS(void* /*pv*/)
{
    const TickType_t period = pdMS_TO_TICKS(20);  // 50 Hz drain rate
    ...
    for (;;) {
        ...
        // Print GPS status to serial every 5 s
        if (millis() - lastPrintMs >= 5000) {
            if (d.valid) {
                Serial.printf("[GPS] Fix  Lat=%.6f  Lon=%.6f  Alt=%.1fm  Sats=%d  HDOP=%.1f  Speed=%.1fkm/h  UTC=%02d:%02d:%02d\n",
...
static void taskSerial(void* /*pv*/)
{
    ...
    uint32_t tick=0;
    Serial.println(F("\n╔══════════════════════════════════════════════════════╗"));
    Serial.println(F("║  FlySky iBUS + MPU-9250 + BMP280 + GPS  v2.2        ║"));
    ...
    for(;;){
        ...
        if(tick%20==0){
            ...
            Serial.printf("[%6lu] %s | R=%+6.1f P=%+6.1f Y=%6.1f | MOT %.2f %.2f %.2f %.2f | "
                          "GPS %s Lat=%.5f Lon=%.5f Alt=%.1fm Sats=%d | mKp=%.2f\n",
                          (unsigned long)tick, ms,
                          s.roll_deg, s.pitch_deg, s.yaw_deg,
                          s.motorFL, s.motorFR, s.motorRL, s.motorRR,
                          s.gps.valid ? "FIX" : "---",
                          s.gps.latitude, s.gps.longitude, s.gps.altitude_m,
                          s.gps.satellites, imu.mahonyKp);
        }
        tick++;
        vTaskDelayUntil(&lastWake,period);
    }
...
void setup()
{
    ...
    // GPS init — UART1 on GPIO13(RX) / GPIO17(TX)
    ...
    xTaskCreatePinnedToCore(taskGPS,   "GPS",    4096, nullptr, 1, &hTaskGPS,   0);  // ← NEW
    // Core 1 tasks
    ...
    Serial.println(F("[BOOT] All tasks running. GPS fix may take 30-90s outdoors."));
}
