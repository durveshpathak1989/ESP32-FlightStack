...
void TelemetryWiFi::_handleTune()
{
    ...
    tryF("alt_hold_kp",              t.has_alt_hold_kp,              t.alt_hold_kp);
    if (!t.has_alt_hold_kp) { tryF("altHoldKp", t.has_alt_hold_kp, t.alt_hold_kp); }
    tryF("alt_hold_kd",              t.has_alt_hold_kd,              t.alt_hold_kd);
    if (!t.has_alt_hold_kd) { tryF("altHoldKd", t.has_alt_hold_kd, t.alt_hold_kd); }
    tryF("alt_hold_max_trim",        t.has_alt_hold_max_trim,        t.alt_hold_max_trim);
    if (!t.has_alt_hold_max_trim) { tryF("altHoldMaxTrim", t.has_alt_hold_max_trim, t.alt_hold_max_trim); }
...
#define JB(key, value) appendJsonBool(j, F(key), (value))
    ...
    JF(",\"batteryAdcV\":", p.battery_adc_v, 3);
    JF(",\"batteryCellV\":", p.battery_cell_v, 3);
    JF(",\"batteryPercent\":", p.battery_percent, 1);
    JB(",\"batteryValid\":", p.battery_valid);
    JB(",\"batteryLow\":", p.battery_low);
    JB(",\"batteryCritical\":", p.battery_critical);
    ...
    JU(",\"tofDistanceMm\":", p.tof_distance_mm);
    JU(",\"tofRangeStatus\":", p.tof_range_status);
    JU(",\"tofObjectCount\":", p.tof_object_count);
    JU(",\"tofStreamCount\":", p.tof_stream_count);
    JF(",\"tofSignalMcps\":", p.tof_signal_mcps, 3);
    JF(",\"tofAmbientMcps\":", p.tof_ambient_mcps, 3);
    JU(",\"tofAgeMs\":", p.tof_age_ms);
    JU(",\"tofLastUpdateMs\":", p.tof_last_update_ms);
    JB(",\"tofReady\":", p.tof_ready);
    JB(",\"tofValid\":", p.tof_valid);
    JF(",\"ekfPosZM\":", p.ekf_pos_z_m, 3);
    JF(",\"ekfVelZMps\":", p.ekf_vel_z_mps, 3);
    JU(",\"tofEkfLastFusedMs\":", p.tof_ekf_last_fused_ms);
    JU(",\"tofEkfAgeMs\":", p.tof_ekf_age_ms);
    JB(",\"tofEkfVelocityAccepted\":", p.tof_ekf_velocity_accepted);
    JB(",\"altitudeHoldRequested\":", p.altitude_hold_requested);
    JB(",\"altitudeHoldReady\":", p.altitude_hold_ready);
    JB(",\"altitudeHoldActive\":", p.altitude_hold_active);
    JF(",\"altitudeHoldSetpointM\":", p.altitude_hold_setpoint_m, 3);
    JF(",\"altitudeHoldErrorM\":", p.altitude_hold_error_m, 3);
    JF(",\"altitudeHoldThrottleTrim\":", p.altitude_hold_throttle_trim, 4);
    ...
    JF(",\"altHoldKp\":", p.alt_hold_kp, 6);
    JF(",\"altHoldKd\":", p.alt_hold_kd, 6);
    JF(",\"altHoldMaxTrim\":", p.alt_hold_max_trim, 6);
